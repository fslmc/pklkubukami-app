...
<section class="section dashboard">
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form class="row-g-3" action="{{ route('projek.update', ['id' => encrypt($projek->id)]) }}" enctype="multipart/form-data" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3 col-12">
                            <label for="judul" class="form-label">Judul</label>
                            <input type="text" class="form-control" id="judul" name="judul" value="{{ old('judul', $projek->judul) }}" placeholder="judul">
                        </div>
                        <div class="mb-3 col-12">
                            <label for="konten" class="form-label">Konten</label>
                            <textarea type="text" class="form-control" id="konten" name="konten" placeholder="konten">{{ old('konten', $projek->konten) }}</textarea>
                        </div>
                        <div class="mb-3 col-12">
                            <label for="thumbnail" class="form-label">Thumbnail</label><br>
                            @if ( $projek->thumbnail )
                                <img src="{{ asset($projek->thumbnail) }}" alt="{{ $projek->judul }}" class="img-fluid w-25">
                            @else
